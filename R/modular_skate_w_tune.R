...
#'     \item{\code{skater_regions}}{The anchor value used.}
...
#' @examples
#' \dontrun{
#' stab <- evaluate_nrst_stability(g, trees, skater_regions = 20,
...
evaluate_nrst_stability <- function(graph,
        ...
        if (verbose) message("  n.rst stability analysis (skater_regions = ",
                             skater_regions, ")")
...
#'
#' For each spanning tree, removes \code{skater_regions - 1} edges (sampled
...
#'   supplied for safety.
#' @param skater_regions Integer scalar \eqn{\ge 2}. Number of regions per tree;
...
#'     \item{\code{skater_regions}}{Number of regions per tree.}
...
compute_ensemble_memberships <- function(trees,
        ...
        if (verbose) {
                msg <- sprintf(
                        "Computing ensemble memberships (n.rst = %d, skater_regions = %d",
                        n.rst, skater_regions
                )
        ...
        memb <- .compute_ensemble_memberships_internal(
                ...
                skater_regions = skater_regions, prior_strength = prior_strength
        )
        ...
        structure(
                list(
                        ...
                        skater_regions = as.integer(skater_regions),
...
print.ensemble_memberships <- function(x, ...) {
        cat(sprintf(
                "<ensemble_memberships>  %d vertices x %d trees | skater_regions = %d\n",
                x$n_vertices, x$n.rst, x$skater_regions
        ))
        invisible(x)
}
...
#' @param skater_regions Integer or integer vector (each \eqn{\ge 2}). Candidate
...
#'   factorial), \code{"sequential"} (tune \code{skater_regions} then
...
tune_skater_con <- function(graph,
        ...
        skater_regions <- sort(unique(as.integer(skater_regions)))
        final_regions  <- sort(unique(as.integer(final_regions)))
        stopifnot(all(skater_regions >= 2), all(final_regions >= 2),
                  all(skater_regions <= trees$n_vertices))
        stopifnot(length(n.rst) == 1L, n.rst >= 2, n.rst <= trees$n_trees)
        ...
        # --- Cache ensemble memberships keyed by skater_regions ------------------
        ...
        score_pair <- function(sr, fr) {
                ...
                data.frame(skater_regions = sr, final_regions = fr, cvis)
        }
        ...
        evaluate_pairs <- function(pairs_df, tick = NULL) {
                ...
                if (length(new_rows) > 0L) {
                        ...
                        col_order <- c("skater_regions", "final_regions", cvi_cols)
        ...
        if (strategy == "grid") {
                pairs <- expand.grid(skater_regions = skater_regions,
                ...
                if (length(skater_regions) > 1L) {
                        if (verbose) message("  Step 1: tuning skater_regions (fr = ", fr_anchor, ")")
                        tick <- if (verbose) .make_progress(length(skater_regions), "skater_regions") else NULL
                        evaluate_pairs(
                                data.frame(skater_regions = skater_regions, final_regions = fr_anchor),
                                tick
                        )
                        best_sr <- best_along("final_regions", fr_anchor, "skater_regions")
                        if (verbose) message("  >> Best skater_regions = ", best_sr)
                } else {
                        best_sr <- skater_regions
                }
                if (length(final_regions) > 1L) {
                        ...
                        evaluate_pairs(
                                data.frame(skater_regions = best_sr, final_regions = final_regions),
                                tick
                        )
                }
                
        } else if (strategy == "iterative") {
                best_sr    <- skater_regions[ceiling(length(skater_regions) / 2)]
                ...
                repeat {
                        ...
                        if (length(skater_regions) > 1L) {
                                if (verbose) message("  Tuning skater_regions (fr = ", best_fr, ")")
                                tick <- if (verbose) .make_progress(length(skater_regions),
                                                                    sprintf("Iter %d: sr", iter)) else NULL
                                evaluate_pairs(
                                        data.frame(skater_regions = skater_regions, final_regions = best_fr),
                                        tick
                                )
                                best_sr <- best_along("final_regions", best_fr, "skater_regions")
                        }
                        if (length(final_regions) > 1L) {
                                ...
                                best_fr <- best_along("skater_regions", best_sr, "final_regions")
                        }
        ...
        tuning_log <- tuning_log[!duplicated(
                tuning_log[, c("skater_regions", "final_regions")]
        ), , drop = FALSE]
        ...
        best_sr  <- tuning_log$skater_regions[best_idx]
        best_fr  <- tuning_log$final_regions[best_idx]
        best_sc  <- tuning_log$score[best_idx]
        
        if (verbose) {
                message(sprintf("\n>> Best: skater_regions = %d, final_regions = %d (%s score = %.4f)",
...
#' @param skater_regions,final_regions,n.rst Integer or integer vectors of
#'   candidate values. If a vector of length > 1 is supplied for
#'   \code{skater_regions} or \code{final_regions}, tuning is triggered; if
...
#'     \item{\code{best_skater_regions}, \code{best_final_regions},
...
skater_con <- function(graph,
        ...
        if (any(skater_regions > igraph::vcount(graph))) {
                stop("`skater_regions` cannot exceed the number of vertices (",
        ...
        if (length(n.rst_values) > 1L) {
                sr_anchor <- skater_regions[ceiling(length(skater_regions) / 2)]
                n.rst_stability <- evaluate_nrst_stability(
                        graph, trees, skater_regions = sr_anchor,
        ...
        if (needs_tuning) {
                ...
                list(
                        ...
                        best_skater_regions = tuning_result$best_skater_regions,
                ...
                list(
                        ...
                        best_skater_regions = skater_regions,
...
#' evaluator. For each tree, removes \code{skater_regions - 1} edges (weighted
...
.compute_ensemble_memberships_internal <- function(graph,
        ...
        n_reg   <- as.integer(skater_regions)
        n_v     <- trees$n_vertices
        weights <- trees$edge_weights
        ep_all  <- trees$edge_endpoints
        
        if (n_reg > n_v) {
                stop(sprintf("skater_regions (%d) cannot exceed the number of vertices (%d).",
                ...
                if (n_to_cut >= length(edge_ids)) {
                        stop(sprintf(
                                paste0("Requested %d cuts but the spanning tree has only %d edges; ",
                                       "reduce skater_regions or supply a connected graph."),
