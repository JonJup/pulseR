...
#' @export
compute_signature <- function(focal, P, graph, Rk, A = NULL,
        ...
        # ---- Distance weights -----------------------------------------------------
        # For the default "none" kernel all weights are 1, so the (per-focal)
        # shortest-path computation is skipped entirely.
        if (kernel == "none") {
                w <- rep.int(1, length(nb_idx))
        } else {
                if (length(sigma) != 1L || is.na(sigma) || sigma <= 0)
                        stop("`sigma` must be a single positive number when kernel != 'none'.",
                             call. = FALSE)
                dists <- igraph::distances(graph, v = focal, to = nb_idx, mode = "all")[1, ]
                w <- switch(kernel,
...
#' @export
compute_all_signatures <- function(P, graph, k, verbose = TRUE,
                                   kernel = c("none", "gaussian", "exponential", "linear"),
                                   sigma  = 1,
                                   digits = 3L) {
        kernel <- match.arg(kernel)
        ...
        for (i in seq_len(n)) {
                sigs[i, ] <- compute_signature(i, P, graph, Rk, A = A,
...
#' @seealso \code{\link{compute_all_signatures}}, \code{\link{cluster_landscapes}}
#' @export
sweep_k_order <- function(P, graph, k_range = 1:5, n_clusters = 2:10,
                          kernel = "none", sigma = 1) {
        if (!igraph::is_igraph(graph))
                stop("`graph` must be an igraph object.", call. = FALSE)
        ...
        for (ki in seq_along(k_range)) {
                ...
                sigs <- compute_all_signatures(P, graph, k, verbose = TRUE,
