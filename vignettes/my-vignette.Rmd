...
But how are you supposed to now how many regions to choose or at which number of random spanning tree results stabalize?
To this end there are hyperparameter tuning facilities included in the function.

```{r}
regions <- skater_con(g_weighted,
                       skater_regions = c(50,70, 125),
                       final_regions = c(2:10),
                       n.rst = c(500, 750, 1000),
                       fuzzyness_parameter = 2,
                       tuning_strategy = c("iterative"),
                       tuning_metric = c("silhouette", "partition_coefficient",
...
plot_fuzzy_area(g$polygons, fuzzy_memberships = regions$memberships, plot = "dominant")
...
fuzzy_sf <- cbind(g$polygons, regions$memberships)
 
# Extract core regions (membership >= 0.8)
cores <- get_core_regions(
  fuzzy_sf,
  membership_cols = paste0("X", 1:ncol(regions$memberships)),
  cutoff = 0.8
)
```
...
rt <- river_types(
  all_data = catchments,
  core_regions = cores,
  membership_id = round(regions$memberships,2),
  membership_cols = paste0("X", 1:ncol(regions$memberships)),
  non_value_cols = "ID",
  n_river_types = 1:3
)
...
To quantify the similarity between river types (modeled as multivariate
normal distributions), use `compare_river_types`, which computes the Bhattacharayya 
distance between each combination of types, within and across regions. 
The Bhattacharyya distance measures how much two probability distributions overlap.
For two multivariate Gaussians, it combines two sources of divergence:
the difference in their means (how far apart the "centers" are) and the 
difference in their covariance structures (how differently shaped and oriented 
their spreads are). A distance of zero means the distributions are identical, 
and larger values indicate less overlap. It's a natural choice for comparing
mixture model components because each component is already parameterized by a
mean vector and covariance matrix.

 
```{r bhattacharyya}
comparison <- compare_river_types(rt$models)
